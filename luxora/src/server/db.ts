    ...
    initPromise = (async () => {
      ...
      if (isPostgresConfigured) {
        try {
          ...
          // Execute idempotent PostgreSQL migrations and seeding
          ...
          const warnMsg = `PostgreSQL connection failed: ${err.message}. Falling back to local persistent SQLite engine.`;
          console.warn('[LUXORA DB] ' + warnMsg);
          db.lastError = warnMsg;
          if (pgPool) {
      ...
      // SQLite initialization (Default or Fallback)
      try {
        db.engine = 'sqlite';
        ...
        db.isConnected = true;
        console.log('[LUXORA DB] SQLite database synchronized and ready.');
      } catch (err: any) {
...
function seedSqliteSync(sqlite: any) {
  ...
  if (!adminRow) {
    ...
    const adminEmail = (process.env.ADMIN_EMAIL || 'admin@luxora.in').trim();
    const initialPassword = process.env.ADMIN_INITIAL_PASSWORD || 'LuxoraAdmin@2026!';
