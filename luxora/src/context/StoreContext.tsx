...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentView, setCurrentViewState] = useState<ViewMode>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (path === '/men' || path === '/women' || path === '/shop') {
        return 'shop';
      }
  ...
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (path.startsWith('/product/')) {
        const identifier = decodeURIComponent(path.replace(/^\/product\//, '').replace(/\/$/, ''));
        const found = LUXORA_PRODUCTS.find(p => p.id.toLowerCase() === identifier.toLowerCase() || (p.slug && p.slug.toLowerCase() === identifier.toLowerCase()));
        return found || null;
      }
    }
    return null;
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (path === '/spadmin' || path.startsWith('/spadmin/')) {
      ...
      } else if (path === '/men' || path === '/women' || path === '/shop') {
        setCurrentViewState('shop');
      } else if (path.startsWith('/product/')) {
        const identifier = decodeURIComponent(path.replace(/^\/product\//, '').replace(/\/$/, ''));
        const found = products.find(p => p.id.toLowerCase() === identifier.toLowerCase() || (p.slug && p.slug.toLowerCase() === identifier.toLowerCase()));
        ...
        setCurrentViewState('lookbook');
      } else if (path === '/about') {
        setCurrentViewState('about');
      } else if (path === '/sustainability') {
        setCurrentViewState('sustainability');
      } else if (path === '/faq') {
        setCurrentViewState('faq');
